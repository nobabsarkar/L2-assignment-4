...
const initiatePaymentIntoDB = async (rentalRequestId, tenantId) => {
...
const validatePayment = async (propertyId, tranId, status, payload) => {
    const response = await axios.post(`https://sandbox.sslcommerz.com/validator/api/validationserverAPI.php?val_id=${payload.val_id}&store_id=${config.ssl_commerz_store_id}&store_passwd=${config.ssl_commerz_store_password}&format=json`, {
        headers: {
            "Content-Type": "application/x-www-form-urlencoded",
        },
    });
...
const getSinglePaymentsFromDB = async (paymentId) => {
